...
// DATA — 일정이 바뀌면 여기만 수정
// ═══════════════════════════════════════════════════════════════════════════
// EVSIS 9모델 수량 — 정본: GS차지비 충전기 모델 현황 시트.
// 아래 표기(모델별 연동 방식 qty · 펌웨어 표 제목 · 전환 준비 체크리스트)는 모두
// 이 한 곳에서 산출한다. 수량 변경 시 여기만 고칠 것.
const EVSIS_QTY = {
  "JC-92B1-7-F1B7": 112,
  "JC-9111KE-TP-BC": 104,
  "JC-91B2-14-0A1": 11,
...
const EVSIS_TOTAL = Object.values(EVSIS_QTY).reduce((a, b) => a + b, 0);
const eq = (model) => String(EVSIS_QTY[model]);
...
const DATA = {
  ...
  keyDates: [
    ...
    { date: "2026-07-20", label: "삼성바이오로직스 P4 주차타워 프록시 테스트 — 시그넷 HB14K(7/15 중단분 재테스트) · FC100K-DCM-PS-H1", track: "transfer" },
  ...
  actions: [
    ...
    { status: "진행 예정", text: "삼성바이오로직스 P4 주차타워 프록시 테스트 — 시그넷 HB14K(7/15 농서동 중단분 재테스트) · FC100K-DCM-PS-H1", owner: "GS차지비", due: "7/20" },
  ...
  proxyInfra: {
    ...
    step2: {
      ...
      items: [
        ...
        { done: false, text: "7/20 삼성바이오로직스 P4 현장에 시그넷 FC100K-DCM-PS-H1 프록시 테스트도 함께 진행 예정" },
  ...
  proxyReadiness: {
    ...
    head: ["제조사", "모델", "유형", "수량(기)", "프록시 연동 테스트", "테스트 일정·현장"],
    rows: [
      ["아이마켓코리아", "운영 충전기 (자체 서버→Proxy)", "기존", "-", "완료", "6/5 개발 · 6~7월 전환"],
      ["이브이시스", "JC-92B1-7-F1B7", "완속 7kW", eq("JC-92B1-7-F1B7"), "완료", "삼성 평택 7/10"],
      ["시그넷", "HB14K-EV-C1C1-G1", "완속 14kW", "142", "예정", "삼성바이오로직스 P4 7/20 (농서동 7/15 중단)"],
      ["시그넷", "HDP300K-DCM", "초급속 300kW", "3", "완료", "농서동 7/15 (모니트)"],
      ["이브이시스", "JC-9111KE-TP-BC", "완속 7kW", eq("JC-9111KE-TP-BC"), "예정", "거제 7/21~23"],
      ["이브이시스", "JC-91B2-14-0A1", "완속 14kW", eq("JC-91B2-14-0A1"), "예정", "거제 7/21~23"],
      ["이브이시스", "JC-96S1-200-0W", "초급속 200kW", eq("JC-96S1-200-0W"), "예정", "거제 7/21~23"],
      ["이브이시스", "JC-9932-100-CU", "급속 100kW", eq("JC-9932-100-CU"), "예정", "거제 7/21~23"],
      ["이브이시스", "JC-92B1-7-01B", "완속 7kW", eq("JC-92B1-7-01B"), "미실시", "-"],
      ["이브이시스", "JC-9931-50-3", "급속 50kW", eq("JC-9931-50-3"), "미실시", "-"],
      ["이브이시스", "JC-9932-100-821", "급속 100kW", eq("JC-9932-100-821"), "미실시", "-"],
      ["이브이시스", "JC-9932-100-8216", "급속 100kW", eq("JC-9932-100-8216"), "미실시", "-"],
      ["시그넷", "FC100K-DCM-PS", "급속 100kW", "1", "미실시", "-"],
      ["시그넷", "FC100K-DCM-PS-H1", "급속 100kW", "7", "예정", "삼성바이오로직스 P4 7/20"],
    ],
  },
  ...
  evsisFw: {
    title: `펌웨어 개발 일정 및 현황 (9모델 · ${EVSIS_TOTAL}기)`,
  ...
  proxyKeep: {
    ...
    head: ["제조사/모델", "수량(기)", "비고"],
    rows: [
      ["시그넷 FC100K-DCM-PS", "1", "급속 100kW · 펌웨어 수정 불가 (설치 2021~2022)"],
      ["시그넷 FC100K-DCM-PS-H1", "7", "급속 100kW · 펌웨어 수정 불가 · 삼성바이오로직스 P4 7/20 프록시 테스트 예정"],
...
function readyCount(status) {
  return DATA.proxyReadiness.rows.filter((r) => r[4] === status).length;
}
...
<main class="doc">
  ...
  <section class="card" id="sec-readiness">
    ...
    <div class="body">
      ...
      ${table(DATA.proxyReadiness, { badgeCols: [4] })}
    </div>
  </section>
